...
const FinanceRefund = ({ refunds, users, listings, payoutMethods, payoutMethodDetails }) => {
    return (
                        ...
                        refunds.map((booking, index) => {
                            ...
                            return (
                                <tr key={booking.id || index} className="text-center border border-gray-300 odd:bg-gray-100">
                                    <td className="border p-2">{listing?.facility_type || "N/A"}</td>
                                    <td className="border p-2">
                                        {booking?.check_in} - {booking?.check_out}
                                    </td>
                                    <td className="border p-2">{booking?.payment_mode || "N/A"}</td>
                                    <td className="border p-2">${booking?.amount || "0.00"}</td>
                                    <td className="border p-2">{host?.name || "N/A"}</td>
